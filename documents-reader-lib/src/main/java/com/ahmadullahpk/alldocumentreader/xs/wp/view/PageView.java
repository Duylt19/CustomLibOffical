...
public class PageView extends AbstractView {
    ...
    private void drawBackground(Canvas canvas, int dx, int dy, float zoom) {
        ...
        if (pageFill != null) {
            BackgroundDrawer.drawBackground(canvas, getControl(), pageNumber, pageFill, rect, null,
                    zoom);
        } else {
     ...
     *
     */
    private void drawShape(Canvas canvas, int originX, int originY, float zoom,
                           boolean drawBehindDocShape) {
    ...
    public long viewToModel(int x, int y, boolean isBack) {
        ...
        if (view != null && y > view.getY()) {
            while (view != null) {
                if (y >= view.getY() && y <
                        view.getY() + view.getHeight()/*.getLayoutSpan(WPViewConstant.Y_AXIS)*/) {
                    break;
                }
                view = view.getNextView();
            }
